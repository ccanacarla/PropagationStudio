...
import { INTERACTION_MODES, DIRECTION_OPTIONS, SPATIAL_MODES, GRID_DEFAULTS, SYNTHETIC_MAP_DEFAULTS, SPATIAL_DEFAULTS, GEOJSON_DEFAULTS } from './constants.js';
...
import { generateRunsPdf } from './export/report-pdf.js';

const $ = s => document.querySelector(s), $$ = s => [...document.querySelectorAll(s)];
const num = (id, fallback = 0) => { const el = $(id), v = Number(el?.value); return Number.isFinite(v) ? v : fallback; };
const esc = s => String(s ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[c]));
const clone = x => structuredClone(x);
const MODE_INFO = { select: ['Selecionar', 'Clique em uma região para inspecionar.'], add_origin: ['Adicionar origem', 'Clique em uma região. Você pode adicionar várias origens.'], add_focus: ['Adicionar foco', 'Clique em qualquer região para programar um foco independente.'], add_jump: ['Adicionar salto', 'Selecione a região de origem do salto e depois uma região não adjacente.'], add_barrier: ['Bloqueio vacinal', 'Clique em regiões para aumentar a vacinação. Com 100%, a região sai da conectividade espacial normal.'], add_preferred: ['Caminho suscetível', 'Clique em uma origem ou foco e depois em regiões adjacentes. Para criar outro caminho a partir da mesma origem/foco, clique nela novamente e continue por outra direção.'], erase: ['Apagar', 'Clique em uma região para remover eventos, caminho e bloqueio associados.'] };

const migratePropagation = (input = {}) => { const base = defaultPropagation(), next = { ...base, ...input, direction: { ...base.direction, ...(input.direction || {}) }, pathSettings: { ...base.pathSettings, ...(input.pathSettings || {}) }, barrierSettings: { ...base.barrierSettings, ...(input.barrierSettings || {}) } }; for (const k of ['origins', 'focuses', 'jumps', 'vaccinationBarriers', 'pathRegions', 'paths']) next[k] = Array.isArray(input[k]) ? input[k] : []; if (!next.vaccinationBarriers.length && Array.isArray(input.barriers)) { const seen = new Set(); for (const b of input.barriers) for (const regionId of [b.sourceRegionId, b.targetRegionId]) if (regionId && !seen.has(regionId)) { seen.add(regionId); next.vaccinationBarriers.push({ id: `migrated-barrier-${regionId}`, regionId, vaccinationCoverage: Math.round((1 - Math.max(0, Math.min(1, Number(b.multiplier ?? 0)))) * 100), enabled: b.enabled !== false }); } } if (!next.pathRegions.length && Array.isArray(input.preferredPaths)) { const map = new Map(); for (const p of input.preferredPaths) for (const regionId of [p.sourceRegionId, p.targetRegionId]) if (regionId) map.set(regionId, Math.max(map.get(regionId) || 1, Math.max(1, Number(p.multiplier ?? 2)))); next.pathRegions = [...map].map(([regionId, susceptibilityMultiplier], i) => ({ id: `migrated-path-${i + 1}`, regionId, susceptibilityMultiplier, enabled: true })); } if (!next.paths.length && next.pathRegions.length) next.paths = [{ id: 'migrated-path-chain-1', sourceRegionId: next.pathRegions[0].regionId, sourceType: null, sourceEventId: null, regionIds: next.pathRegions.map(item => item.regionId).filter(Boolean), enabled: true }]; return next; };
...
class App {
  ...
  bind() {
    ...
    for (const id of ['#btn-simulate', '#btn-simulate-main', '#btn-simulate-timeline']) $(id).addEventListener('click', () => this.simulate());
    ...
    $('#run-notes').addEventListener('change', e => { const run = state.currentRun(); if (run) state.updateRun(run.id, { notes: e.target.value }); }); $('#btn-approve-run').addEventListener('click', () => this.setRunStatus('approved')); $('#btn-reject-run').addEventListener('click', () => this.setRunStatus('rejected')); $('#btn-export-all').addEventListener('click', () => this.exportAllRunFiles()); $('#btn-export-report-pdf').addEventListener('click', () => this.exportRunsPdfReport()); $$('[data-export]').forEach(b => b.addEventListener('click', () => this.exportRun(b.dataset.export))); $('#analysis-threshold').addEventListener('change', () => this.refreshAnalysis()); $('#analysis-reference').addEventListener('change', () => this.refreshAnalysis());
  }
  ...
  syncViewportLabel() { const run = state.viewRun?.(); if (!run) { $('#grid-dim-label').textContent = state.space.mode === SPATIAL_MODES.GRID ? `Grid · ${state.grid.rows} × ${state.grid.columns}` : state.space.mode === SPATIAL_MODES.SYNTHETIC_MAP ? `Mapa sintético · ${state.regions.size}` : `Mapa · ${state.regions.size} regiões`; return; } const mode = run.space?.mode || SPATIAL_MODES.GRID; const count = (run.regions || []).length; const size = mode === SPATIAL_MODES.GRID ? `Grid · ${run.grid?.rows ?? '—'} × ${run.grid?.columns ?? '—'}` : mode === SPATIAL_MODES.SYNTHETIC_MAP ? `Mapa sintético · ${count}` : `Mapa · ${count} regiões`; $('#grid-dim-label').textContent = `${run.name || run.id} · ${size}`; }
  ...
  onState(type, data) {
    ...
    if (type === 'RUN_SELECTED') { this.pendingSpaceMode = state.space.mode; this.pendingGeoJSON = null; this.pendingGeoJSONName = ''; $('#geojson-import-config').hidden = true; this.syncControls(); this.renderScenario(); this.hideVisualizationInspector(); setTimeout(() => { this.syncViewportLabel(); this.renderer.fit(); }, 0); }
  ...
  syncSpaceControls() { const mode = this.pendingSpaceMode || state.space.mode; $$('.space-mode-card').forEach(b => b.classList.toggle('active', b.dataset.spaceMode === mode)); $('#grid-space-fields').hidden = mode !== SPATIAL_MODES.GRID; $('#synthetic-space-fields').hidden = mode !== SPATIAL_MODES.SYNTHETIC_MAP; $('#geojson-space-fields').hidden = mode !== SPATIAL_MODES.GEOJSON; const syn = state.space.synthetic || {}, geo = state.space.geojson || {}, topology = geo.topology || {}; $('#synthetic-count').min = SYNTHETIC_MAP_DEFAULTS.MIN_REGIONS; $('#synthetic-count').max = SYNTHETIC_MAP_DEFAULTS.MAX_REGIONS; $('#synthetic-count').value = syn.regionCount ?? SYNTHETIC_MAP_DEFAULTS.REGION_COUNT; $('#synthetic-seed').value = syn.spatialSeed ?? SYNTHETIC_MAP_DEFAULTS.SPATIAL_SEED; $('#synthetic-irregularity').value = syn.irregularity ?? SYNTHETIC_MAP_DEFAULTS.IRREGULARITY; $('#synthetic-pop').value = syn.defaultPopulation ?? SPATIAL_DEFAULTS.DEFAULT_POPULATION; $('#geojson-default-pop').value = geo.defaultPopulation ?? GEOJSON_DEFAULTS.DEFAULT_POPULATION; $('#map-adjacency-method').value = topology.method || 'shared_border'; $('#map-border-tolerance').value = topology.tolerance ?? ''; $('#map-nearest-k').value = topology.nearestK ?? 4; let label = ''; if (state.space.mode === SPATIAL_MODES.GRID) label = `Grid · ${state.grid.rows} × ${state.grid.columns} · ${state.regions.size} regiões`; else if (state.space.mode === SPATIAL_MODES.SYNTHETIC_MAP) label = `Mapa sintético · ${state.regions.size} regiões · seed espacial ${state.space.synthetic?.spatialSeed}`; else label = `Mapa importado · ${esc(state.space.geojson?.sourceName || 'GeoJSON')} · ${state.regions.size} regiões${state.space.geojson?.stats ? ` · ${state.space.geojson.stats.isolated} isoladas` : ''}`; $('#active-space-summary').innerHTML = `<strong>Espaço ativo</strong><span>${label}</span>`; $('#grid-dim-label').textContent = state.space.mode === SPATIAL_MODES.GRID ? `Grid · ${state.grid.rows} × ${state.grid.columns}` : state.space.mode === SPATIAL_MODES.SYNTHETIC_MAP ? `Mapa sintético · ${state.regions.size}` : `Mapa · ${state.regions.size} regiões`; this.toggleAdjacencyOptions(); }
  selectSpaceMode(mode) { this.pendingSpaceMode = mode; this.syncSpaceControls(); }
  syncGridControls() { const g = state.grid; $('#grid-rows').min = GRID_DEFAULTS.MIN_ROWS; $('#grid-rows').max = GRID_DEFAULTS.MAX_ROWS; $('#grid-cols').min = GRID_DEFAULTS.MIN_ROWS; $('#grid-cols').max = GRID_DEFAULTS.MAX_ROWS; $('#grid-rows').value = g.rows; $('#grid-cols').value = g.columns; $('#grid-neighborhood').value = g.neighborhood; $('#grid-border').value = g.borderMode; $('#grid-random-pop').checked = !!g.randomizePopulation; $('#grid-pop').value = g.defaultPopulation; $('#grid-min-pop').value = g.minPopulation; $('#grid-max-pop').value = g.maxPopulation; $('#grid-pop-seed').value = g.populationSeed; this.togglePopulationFields(); }
  ...
  applyGrid() { const rows = Math.max(GRID_DEFAULTS.MIN_ROWS, Math.min(GRID_DEFAULTS.MAX_ROWS, num('#grid-rows', GRID_DEFAULTS.DEFAULT_ROWS))), columns = Math.max(GRID_DEFAULTS.MIN_ROWS, Math.min(GRID_DEFAULTS.MAX_ROWS, num('#grid-cols', GRID_DEFAULTS.DEFAULT_COLS))); state.grid = { ...state.grid, rows, columns, neighborhood: $('#grid-neighborhood').value, borderMode: $('#grid-border').value, randomizePopulation: $('#grid-random-pop').checked, defaultPopulation: Math.max(1, num('#grid-pop', SPATIAL_DEFAULTS.DEFAULT_POPULATION)), minPopulation: Math.max(1, num('#grid-min-pop', GRID_DEFAULTS.MIN_POPULATION)), maxPopulation: Math.max(1, num('#grid-max-pop', GRID_DEFAULTS.MAX_POPULATION)), populationSeed: num('#grid-pop-seed', GRID_DEFAULTS.POPULATION_SEED) }; if (state.grid.maxPopulation < state.grid.minPopulation) state.grid.maxPopulation = state.grid.minPopulation; state.rebuildGrid(); this.syncControls(); this.renderer.fit(); this.toast('success', 'Grid recriado', `${rows} × ${columns} regiões disponíveis.`); }
  buildSynthetic() { const cfg = { regionCount: Math.max(SYNTHETIC_MAP_DEFAULTS.MIN_REGIONS, Math.min(SYNTHETIC_MAP_DEFAULTS.MAX_REGIONS, num('#synthetic-count', SYNTHETIC_MAP_DEFAULTS.REGION_COUNT))), spatialSeed: num('#synthetic-seed', SYNTHETIC_MAP_DEFAULTS.SPATIAL_SEED), irregularity: Math.max(0, Math.min(1, num('#synthetic-irregularity', SYNTHETIC_MAP_DEFAULTS.IRREGULARITY))), defaultPopulation: Math.max(1, num('#synthetic-pop', SPATIAL_DEFAULTS.DEFAULT_POPULATION)) }; state.rebuildSyntheticMap(cfg); this.syncControls(); this.renderer.fit(); this.toast('success', 'Mapa sintético criado', `${state.regions.size} regiões irregulares foram geradas com a seed espacial ${cfg.spatialSeed}.`); }
  async readGeoJSON(file) { if (!file) return; try { const data = JSON.parse(await file.text()), info = validateGeoJSON(data), inferred = inferGeoJSONFields(info.propertyKeys); this.pendingGeoJSON = data; this.pendingGeoJSONName = file.name; const make = (sel, empty, label) => { const el = $(sel); el.innerHTML = `<option value="">${empty}</option>` + info.propertyKeys.map(k => `<option value="${esc(k)}">${esc(k)}</option>`).join(''); if (label && info.propertyKeys.includes(label)) el.value = label; }; make('#geojson-id-prop', 'Gerar automaticamente', inferred.idProperty); make('#geojson-name-prop', 'Usar o ID', inferred.nameProperty); make('#geojson-pop-prop', 'Usar população padrão', inferred.populationProperty); $('#geojson-import-config').hidden = false; $('#geojson-summary').innerHTML = `<strong>${esc(file.name)}</strong><span>${info.features.length} regiões válidas · ${info.propertyKeys.length} atributos${info.invalidFeatureIndexes.length ? ` · ${info.invalidFeatureIndexes.length} feições ignoradas` : ''}</span>`; if (info.features.length > GEOJSON_DEFAULTS.LARGE_MAP_THRESHOLD) this.toast('warning', 'Mapa grande', 'Mais de 500 regiões podem deixar a construção de fronteiras e a animação mais lentas.'); } catch (error) { this.pendingGeoJSON = null; $('#geojson-import-config').hidden = true; this.toast('danger', 'GeoJSON inválido', error.message); } finally { $('#geojson-file').value = ''; } }
  applyGeoJSON() { if (!this.pendingGeoJSON) { this.toast('warning', 'Nenhum mapa selecionado', 'Escolha um arquivo GeoJSON primeiro.'); return; } try { const mapping = { idProperty: $('#geojson-id-prop').value, nameProperty: $('#geojson-name-prop').value, populationProperty: $('#geojson-pop-prop').value }, regions = normalizeGeoJSON(this.pendingGeoJSON, mapping, { defaultPopulation: Math.max(1, num('#geojson-default-pop', GEOJSON_DEFAULTS.DEFAULT_POPULATION)) }), topology = buildGeoJSONTopology(regions, { method: $('#map-adjacency-method').value, tolerance: $('#map-border-tolerance').value, nearestK: num('#map-nearest-k', 4) }); state.applyGeoJSONRegions(topology.regions, { sourceName: this.pendingGeoJSONName, defaultPopulation: Math.max(1, num('#geojson-default-pop', GEOJSON_DEFAULTS.DEFAULT_POPULATION)), mapping, topology: { method: topology.method, tolerance: topology.tolerance, nearestK: num('#map-nearest-k', 4) }, stats: topology.stats }); this.syncControls(); this.renderer.fit(); this.toast(topology.stats.isolated ? 'warning' : 'success', 'Mapa importado', `${topology.stats.regions} regiões · grau médio ${topology.stats.meanDegree.toFixed(1)} · ${topology.stats.isolated} regiões isoladas.`); } catch (error) { console.error(error); this.toast('danger', 'Falha ao construir o mapa', error.message); } }
  ...
  renderMode() { const [title, help] = MODE_INFO[state.interactionMode] || MODE_INFO.select; $('#mode-title').textContent = title; const pending = state.pendingSourceId ? (state.interactionMode === INTERACTION_MODES.ADD_PREFERRED ? ` Última região: ${state.pendingSourceId}. Você pode clicar em uma origem/foco novamente para iniciar outra ramificação.` : ` Primeira região: ${state.pendingSourceId}.`) : ''; $('#mode-help').textContent = help + pending; $('#btn-cancel-mode').hidden = state.interactionMode === INTERACTION_MODES.SELECT; $$('.tool').forEach(b => b.classList.toggle('active', b.dataset.mode === state.interactionMode)); }
  ...
  regionLabel(id) { const r = state.regions.get(id); return r ? `${r.name || id} (${id})` : id; }
  ...
  handleHover(id) { if (!id) { $('#grid-canvas').title = ''; return; } const run = state.viewRun?.(), regions = run ? this.runRegionMap(run) : state.regions, propagation = run?.propagation || state.propagation, r = run?.history?.[state.currentTimeStep]?.get(id) || regions.get(id); if (r) { const path = propagation.pathRegions?.find(x => x.regionId === id && x.enabled !== false), barrier = propagation.vaccinationBarriers?.find(x => x.regionId === id && x.enabled !== false), meta = regions.get(id); $('#grid-canvas').title = `${meta?.name || id} · ${id} · Pop ${r.population} · S ${r.susceptible} · I ${r.infected} · R ${r.recovered} · V ${r.vaccinated}${path ? ` · caminho χ×${path.susceptibilityMultiplier}` : ''}${barrier ? ` · bloqueio V${Math.round(barrier.vaccinationCoverage)}%` : ''}`; } }
  handleSpatialClick(id) { state.setSelectedRegion(id); const mode = state.interactionMode; if (mode === INTERACTION_MODES.SELECT) { state.setPanel('region'); return; } if (mode === INTERACTION_MODES.ADD_ORIGIN) { if (this.isFullyBlocked(id)) { this.toast('warning', 'Região totalmente vacinada', 'Uma origem não pode infectar uma região com bloqueio de 100%.'); return; } if (state.addOrigin(id)) this.toast('success', 'Origem adicionada', this.regionLabel(id)); else this.toast('warning', 'Origem existente', this.regionLabel(id)); return; } if (mode === INTERACTION_MODES.ADD_FOCUS) { if (this.isFullyBlocked(id)) { this.toast('warning', 'Região totalmente vacinada', 'Um foco não pode infectar uma região com bloqueio de 100%.'); return; } state.addFocus(id); this.toast('success', 'Foco adicionado', 'Edite instante e intensidade no painel Propagação.'); return; } if (mode === INTERACTION_MODES.ADD_BARRIER) { const added = state.addVaccinationBarrier(id); this.toast('success', added ? 'Bloqueio adicionado' : 'Bloqueio atualizado', `${this.regionLabel(id)} recebeu ${Math.round(state.propagation.barrierSettings.vaccinationCoverage)}% de vacinação inicial.`); return; } if (mode === INTERACTION_MODES.ERASE) { state.eraseRegion(id); this.toast('success', 'Elementos removidos', this.regionLabel(id)); return; } if (mode === INTERACTION_MODES.ADD_PREFERRED) { if (this.isFullyBlocked(id)) { this.toast('warning', 'Região bloqueada', 'Não faz sentido marcar como caminho uma região 100% vacinada.'); return; } const origin = state.propagation.origins.find(event => event.regionId === id && event.enabled !== false), focus = state.propagation.focuses.find(event => event.regionId === id && event.enabled !== false), anchor = origin ? { sourceType: 'origin', sourceEventId: origin.id } : (focus ? { sourceType: 'focus', sourceEventId: focus.id } : null); if (!state.pendingSourceId || !state.pendingPathId) { state.startPath(id, anchor || {}); state.notify('MODE_CHANGE'); return; } if (anchor) { state.startPath(id, anchor); state.notify('MODE_CHANGE'); this.toast('success', 'Novo caminho iniciado', `Nova ramificação a partir de ${this.regionLabel(id)}.`); return; } const source = state.pendingSourceId, target = id; if (source === target) { this.toast('warning', 'Seleção repetida', 'Escolha uma região adjacente ou clique em uma origem/foco para iniciar outro caminho.'); return; } if (!this.isAdjacent(source, target)) { this.toast('warning', 'Caminho inválido', 'Continue o caminho por uma região adjacente ou clique em uma origem/foco para iniciar outra ramificação.'); return; } state.extendPath(state.pendingPathId, target); state.notify('MODE_CHANGE'); return; } if (mode === INTERACTION_MODES.ADD_JUMP) { if (!state.pendingSourceId) { state.pendingSourceId = id; state.notify('MODE_CHANGE'); return; } const source = state.pendingSourceId, target = id; state.pendingSourceId = null; if (source === target) { this.toast('warning', 'Seleção inválida', 'Origem e destino precisam ser diferentes.'); state.notify('MODE_CHANGE'); return; } if (this.isAdjacent(source, target)) this.toast('warning', 'Salto inválido', 'Use saltos apenas entre regiões não adjacentes.'); else { state.addJump(source, target); this.toast('success', 'Salto adicionado', `${this.regionLabel(source)} → ${this.regionLabel(target)}`); } state.notify('MODE_CHANGE'); } }
  renderScenario() { const p = state.propagation, pathCount = (p.paths || []).length || ((p.pathRegions || []).length ? 1 : 0); $('#count-origins').textContent = p.origins.length; $('#count-focuses').textContent = p.focuses.length; $('#count-jumps').textContent = p.jumps.length; $('#count-barriers').textContent = (p.vaccinationBarriers || []).length; $('#count-paths').textContent = pathCount; this.syncDirectionControls(); this.syncPropagationSettings(); $('#scenario-lists').innerHTML = [this.eventSection('Origens', 'origins', p.origins), this.eventSection('Focos', 'focuses', p.focuses), this.eventSection('Saltos', 'jumps', p.jumps), this.eventSection('Bloqueios vacinais', 'vaccinationBarriers', p.vaccinationBarriers || []), this.eventSection('Regiões do caminho', 'pathRegions', p.pathRegions || [])].join(''); }
  eventSection(title, kind, list) { if (!list.length) return `<details><summary>${title} · 0</summary><p class="hint">Use a barra de ferramentas para adicionar.</p></details>`; return `<details open><summary>${title} · ${list.length}</summary>${list.map((x, i) => this.eventCard(kind, x, i)).join('')}</details>`; }
  regionSelect(kind, id, field, value, label) { const options = [...state.regions.values()].map(r => `<option value="${esc(r.id)}" ${r.id === value ? 'selected' : ''}>${esc(r.name || r.id)} · ${esc(r.id)}</option>`).join(''); return `<label>${label}<select data-kind="${kind}" data-id="${id}" data-field="${field}">${options}</select></label>`; }
  eventCard(kind, x, i) { const field = (name, label, value, type = 'number', extra = '') => `<label>${label}<input data-kind="${kind}" data-id="${x.id}" data-field="${name}" type="${type}" value="${esc(value)}" ${extra}></label>`, check = (name, label, value) => `<label class="check-row"><input data-kind="${kind}" data-id="${x.id}" data-field="${name}" type="checkbox" ${value ? 'checked' : ''}> ${label}</label>`; let body = '', title = ''; if (kind === 'origins' || kind === 'focuses') { body = `<div class="mini-grid">${this.regionSelect(kind, x.id, 'regionId', x.regionId, 'Região')}${field('startTime', 'Início', x.startTime)}${field('infectedCount', 'Infectados', x.infectedCount)}${field('duration', 'Duração', x.duration)}${check('enabled', 'Ativo', x.enabled !== false)}</div>`; title = kind === 'origins' ? `Origem ${i + 1}` : `Foco ${i + 1}`; } if (kind === 'jumps') { body = `<div class="mini-grid">${this.regionSelect(kind, x.id, 'sourceRegionId', x.sourceRegionId, 'Origem')}${this.regionSelect(kind, x.id, 'targetRegionId', x.targetRegionId, 'Destino')}${field('startTime', 'Início', x.startTime)}${field('probability', 'Probabilidade', x.probability, 'number', 'min="0" max="1" step="0.05"')}${field('infectedCount', 'Infectados', x.infectedCount)}${field('interval', 'Intervalo', x.interval)}${check('recurring', 'Recorrente', x.recurring)}${check('enabled', 'Ativo', x.enabled !== false)}</div>`; title = `${this.regionLabel(x.sourceRegionId)} → ${this.regionLabel(x.targetRegionId)}`; } if (kind === 'vaccinationBarriers') { const coverage = Math.max(0, Math.min(100, Number(x.vaccinationCoverage ?? 100))); body = `<div class="mini-grid">${this.regionSelect(kind, x.id, 'regionId', x.regionId, 'Região')}${field('vaccinationCoverage', 'Vacinação inicial (%)', coverage, 'number', 'min="0" max="100" step="1"')}${check('enabled', 'Ativo', x.enabled !== false)}</div><div class="barrier-status ${coverage >= 100 ? '' : 'partial'}">${coverage >= 100 ? 'BLOQUEIO TOTAL' : `V ${Math.round(coverage)}%`}</div><p class="hint">Em 100%, a região é removida das conexões espaciais normais.</p>`; title = `Bloqueio · ${this.regionLabel(x.regionId)}`; } if (kind === 'pathRegions') { const susceptibility = Math.max(0, Number(x.susceptibilityMultiplier ?? 2)); body = `<div class="mini-grid">${this.regionSelect(kind, x.id, 'regionId', x.regionId, 'Região')}${field('susceptibilityMultiplier', 'Suscetibilidade (×)', susceptibility, 'number', 'min="1" max="10" step="0.1"')}${check('enabled', 'Ativo', x.enabled !== false)}</div><div class="path-status">χ ×${susceptibility.toFixed(1)}</div>`; title = `Caminho · ${this.regionLabel(x.regionId)}`; } return `<div class="event-card"><div class="event-card-head"><h4>${esc(title)}</h4><button class="remove" data-remove-kind="${kind}" data-remove-id="${x.id}">Remover</button></div>${body}</div>`; }
  editScenarioItem(e) { const el = e.target; if (!el.dataset.kind) return; const { kind, id, field } = el.dataset; let value = el.type === 'checkbox' ? el.checked : el.value; if (el.type === 'number') value = Number(value); if (kind === 'vaccinationBarriers' && field === 'vaccinationCoverage') value = Math.max(0, Math.min(100, Number(value) || 0)); if (kind === 'pathRegions' && field === 'susceptibilityMultiplier') value = Math.max(1, Math.min(10, Number(value) || 1)); const item = state.propagation[kind]?.find(x => x.id === id); if (item && kind === 'jumps' && ['sourceRegionId', 'targetRegionId'].includes(field)) { const source = field === 'sourceRegionId' ? value : item.sourceRegionId, target = field === 'targetRegionId' ? value : item.targetRegionId; if (source === target) { this.toast('warning', 'Conexão inválida', 'Origem e destino precisam ser diferentes.'); this.renderScenario(); return; } if (this.isAdjacent(source, target)) { this.toast('warning', 'Salto inválido', 'Saltos devem ligar regiões não adjacentes.'); this.renderScenario(); return; } } if (item && ['vaccinationBarriers', 'pathRegions'].includes(kind) && field === 'regionId' && state.propagation[kind].some(x => x.id !== id && x.regionId === value)) { this.toast('warning', 'Região já marcada', 'Essa região já possui essa propriedade.'); this.renderScenario(); return; } state.updateEvent(kind, id, { [field]: value }); }
  renderSeedSummary() { const values = [...state.regions.values()]; if (!values.length) return; const seeded = values.filter(r => r.initialConditionMode !== 'manual'), vacc = values.map(r => r.population > 0 ? 100 * r.vaccinated / r.population : 0), minV = Math.min(...vacc), maxV = Math.max(...vacc), avgV = vacc.reduce((a, b) => a + b, 0) / vacc.length; $('#seeded-conditions-summary').innerHTML = `<strong>Seed ${esc(state.simulationConfig.seed)}</strong><span>${seeded.length} regiões geradas · V médio ${avgV.toFixed(1)}% · intervalo ${minV.toFixed(1)}–${maxV.toFixed(1)}%</span>`; }
  renderRegion() {
    const id = state.selectedRegionId, form = $('#region-form'), run = state.viewRun?.(), regions = run ? this.runRegionMap(run) : state.regions;
    if (!id || !regions.has(id)) { form.hidden = true; $('#region-caption').textContent = run ? `Visualizando ${run.name || run.id}. Clique em uma região para inspecionar o instante atual.` : 'Selecione uma região no espaço.'; return; }
    const meta = regions.get(id), frameRegion = run?.history?.[state.currentTimeStep]?.get(id), r = frameRegion ? { ...meta, ...frameRegion } : meta, propagation = run?.propagation || state.propagation, space = run?.space || state.space;
    form.hidden = false;
    $('#region-caption').textContent = space.mode === SPATIAL_MODES.GRID ? `${r.name || id} · ${id} · linha ${r.row}, coluna ${r.column}${run ? ` · ${run.name || run.id} · t=${state.currentTimeStep}` : ''}` : `${r.name || id} · ${id} · ${(r.neighbors || []).length} vizinhos${run ? ` · ${run.name || run.id} · t=${state.currentTimeStep}` : ''}`;
    $('#region-manual-initial').checked = r.initialConditionMode === 'manual'; $('#region-pop').value = r.population; $('#region-s').value = r.susceptible; $('#region-i').value = r.infected; $('#region-r').value = r.recovered; $('#region-v').value = r.vaccinated; $('#region-susc-m').value = r.localParameters?.susceptibilityMultiplier ?? 1; $('#region-beta-m').value = r.localParameters?.betaMultiplier ?? 1; $('#region-gamma-m').value = r.localParameters?.gammaMultiplier ?? 1; $('#region-nu-m').value = r.localParameters?.vaccinationMultiplier ?? 1; $('#region-mobility-m').value = r.localParameters?.mobilityMultiplier ?? 1;
    const path = propagation.pathRegions?.find(x => x.regionId === id && x.enabled !== false), barrier = propagation.vaccinationBarriers?.find(x => x.regionId === id && x.enabled !== false);
    $('#region-derived-info').innerHTML = `<strong>${run ? `Snapshot da execução · t=${state.currentTimeStep}` : (r.initialConditionMode === 'manual' ? 'Condição inicial manual' : 'Condição inicial gerada pela semente')}</strong><span>V ${r.population ? ((r.vaccinated / r.population) * 100).toFixed(1) : '0.0'}%${path ? ` · caminho χ ×${Number(path.susceptibilityMultiplier).toFixed(1)}` : ''}${barrier ? ` · bloqueio V ${Math.round(barrier.vaccinationCoverage)}%` : ''}</span>${space.mode === SPATIAL_MODES.GEOJSON && Object.keys(meta.sourceProperties || {}).length ? `<span class="source-property">Atributos do mapa preservados no projeto e no GeoJSON exportado.</span>` : ''}`;
    const controls = form.querySelectorAll('input, select, textarea, button'); controls.forEach(el => { if (el.id !== 'btn-save-region') el.disabled = !!run; }); $('#btn-save-region').hidden = !!run;
  }
  ...
  renderRuns() {
    ...
    const run = state.currentRun();
    const pdfButton = $('#btn-export-report-pdf'); if (pdfButton) pdfButton.disabled = !state.runs.length;
    $('#selected-run-actions').hidden = !run;
    if (run) $('#run-notes').value = run.notes || '';
    this.renderRunConfiguration(run);
  }
  renderRunConfiguration(run) {
    const box = $('#run-config-summary');
    if (!box) return;
    if (!run) { box.innerHTML = ''; return; }
    const cfg = run.simulationConfig || {}, propagation = run.propagation || {}, grid = run.grid || {}, space = run.space || {};
    const mode = space.mode === SPATIAL_MODES.SYNTHETIC_MAP ? 'Mapa sintético' : space.mode === SPATIAL_MODES.GEOJSON ? 'GeoJSON' : 'Grid';
    const size = space.mode === SPATIAL_MODES.GRID ? `${grid.rows ?? '—'} × ${grid.columns ?? '—'}` : `${(run.regions || []).length} regiões`;
    const created = run.createdAt && !Number.isNaN(Date.parse(run.createdAt)) ? new Date(run.createdAt).toLocaleString('pt-BR') : '—';
    const direction = propagation.direction?.enabled === false ? 'Desativada' : `${propagation.direction?.direction || 'radial'} · ${propagation.direction?.directionProfile || '—'}`;
    const item = (label, value) => `<div class="run-config-item"><span>${esc(label)}</span><strong>${esc(value ?? '—')}</strong></div>`;
    box.innerHTML = [
      item('Criada em', created), item('Espaço', `${mode} · ${size}`), item('Seed', run.seed ?? cfg.seed ?? '—'), item('Passos', cfg.timeSteps ?? (run.history?.length ? run.history.length - 1 : '—')),
      item('β', cfg.beta ?? '—'), item('γ', cfg.gamma ?? '—'), item('ν', cfg.nu ?? '—'), item('Mobilidade', cfg.mobility ?? '—'),
      item('Origens', propagation.origins?.length || 0), item('Focos', propagation.focuses?.length || 0), item('Saltos', propagation.jumps?.length || 0), item('Bloqueios', propagation.vaccinationBarriers?.length || 0),
      item('Caminhos', propagation.paths?.length || (propagation.pathRegions?.length ? 1 : 0)), item('Direção', direction)
    ].join('');
  }
  ...
  renderAnalysis() { const run = state.currentRun(), box = $('#analysis-panel'); if (!run) { box.className = 'analysis-panel empty-state'; box.textContent = 'Execute uma simulação para calcular métricas.'; return; } const regions = this.runRegionMap(run); box.className = 'analysis-panel'; const s = run.summary || {}, dir = s.observedDirection || {}, corr = (s.topCorrelations || []).map(x => `<tr><td>${esc(regions.get(x.regionId)?.name || x.regionId)}</td><td>${Number(x.correlation).toFixed(3)}</td></tr>`).join(''); box.innerHTML = `<div class="analysis-grid"><div class="metric-card"><strong>${s.reachedRegions ?? 0}/${s.totalRegions ?? 0}</strong><span>regiões atingidas</span></div><div class="metric-card"><strong>t=${s.duration ?? 0}</strong><span>duração com infectados</span></div><div class="metric-card"><strong>${s.peakRegion?.infected ?? 0}</strong><span>maior pico regional · ${esc(regions.get(s.peakRegion?.regionId)?.name || s.peakRegion?.regionId || '—')} em t=${s.peakRegion?.time ?? 0}</span></div><div class="metric-card"><strong>${s.executedJumps ?? 0}</strong><span>saltos executados</span></div><div class="metric-card"><strong>${esc(dir.direction || 'indefinida')}</strong><span>direção observada · confiança ${Number(dir.confidence || 0).toFixed(2)}</span></div><div class="metric-card"><strong>${Math.round((s.attackReach || 0) * 100)}%</strong><span>cobertura espacial</span></div></div>${corr ? `<h3>Regiões mais correlacionadas</h3><table class="analysis-table"><thead><tr><th>Região</th><th>Correlação</th></tr></thead><tbody>${corr}</tbody></table>` : '<p class="hint">Escolha uma região de referência para calcular correlações.</p>'}`; }
  exportRun(type) { const run = state.currentRun(); if (!run) return; const base = this.safeName(`${state.project.name}-${run.name || run.id}`); if (type === 'run-json') downloadText(`${base}-simulation.json`, runJSON(run), 'application/json;charset=utf-8'); if (type === 'temporal') downloadText(`${base}-temporal.csv`, temporalCSV(run), 'text/csv;charset=utf-8'); if (type === 'regions') downloadText(`${base}-regions.csv`, regionsCSV(run), 'text/csv;charset=utf-8'); if (type === 'events') downloadText(`${base}-events.csv`, eventsCSV(run), 'text/csv;charset=utf-8'); if (type === 'edges') downloadText(`${base}-edges.csv`, edgesCSV(run), 'text/csv;charset=utf-8'); if (type === 'summary') downloadText(`${base}-summary.json`, JSON.stringify(run.summary, null, 2), 'application/json;charset=utf-8'); if (type === 'geojson') { const geo = regionsGeoJSON(run); if (geo) downloadText(`${base}-regions.geojson`, geo, 'application/geo+json;charset=utf-8'); } }
  async exportRunsPdfReport() { const button = $('#btn-export-report-pdf'); if (!state.runs.length) { this.toast('warning', 'Sem execuções', 'Execute pelo menos uma simulação antes de gerar o relatório.'); return; } const originalLabel = button.textContent; button.disabled = true; button.textContent = 'Montando relatório...'; try { await generateRunsPdf({ project: clone(state.project), runs: clone(state.runs), onProgress: ({ phase }) => { button.textContent = phase === 'pdf' ? 'Abrindo relatório...' : 'Montando relatório...'; } }); this.toast('success', 'Relatório aberto', 'O relatório visual foi aberto em uma nova janela. Use “Imprimir / Salvar PDF” quando quiser exportá-lo.'); } catch (error) { console.error(error); this.toast('danger', 'Falha ao gerar relatório', error.message || String(error)); } finally { button.disabled = !state.runs.length; button.textContent = originalLabel; } }
  async exportAllRunFiles() { const run = state.currentRun(), button = $('#btn-export-all'); if (!run) { this.toast('warning', 'Nenhuma execução selecionada', 'Selecione uma execução antes de fazer o download.'); return; } if (!window.JSZip) { this.toast('danger', 'Falha no download', 'A biblioteca de compactação não foi carregada.'); return; } const originalLabel = button.textContent; button.disabled = true; button.textContent = 'Preparando...'; try { const zip = new window.JSZip(), files = { 'simulation.json': runJSON(run), 'temporal.csv': temporalCSV(run), 'regions.csv': regionsCSV(run), 'events.csv': eventsCSV(run), 'edges.csv': edgesCSV(run), 'summary.json': JSON.stringify(run.summary || {}, null, 2) }; for (const [name, content] of Object.entries(files)) zip.file(name, content); const geo = regionsGeoJSON(run); if (geo) zip.file('regions.geojson', geo); const blob = await zip.generateAsync({ type: 'blob', compression: 'DEFLATE', compressionOptions: { level: 6 } }); downloadBlob(`${this.safeName(`${state.project.name}-${run.name || run.id}`)}.zip`, blob); this.toast('success', 'Download concluído', 'A execução completa foi compactada em um arquivo ZIP.'); } catch (error) { console.error(error); this.toast('danger', 'Falha no download', `Não foi possível compactar os arquivos da execução: ${error.message || error}`); } finally { button.disabled = false; button.textContent = originalLabel; } }
  async importProject(file) {
    ...
    try {
      ...
      const rawRegions = Array.isArray(data.regions) ? data.regions : Object.values(data.regions || {});
      const importedRegions = new Map(rawRegions.filter(r => r?.id).map(r => [r.id, { ...r, spatialX: Number.isFinite(Number(r.spatialX)) ? Number(r.spatialX) : Number(r.column), spatialY: Number.isFinite(Number(r.spatialY)) ? Number(r.spatialY) : Number(r.row), neighbors: [...(r.neighbors || [])], sourceProperties: { ...(r.sourceProperties || {}) }, initialConditionMode: r.initialConditionMode || 'seeded', localParameters: { susceptibilityMultiplier: 1, ...(r.localParameters || {}) } }]));
      ...
      const restored = deserializeRuns(data.runs, { space: state.space, grid: state.grid, simulationConfig: state.simulationConfig, propagation: state.propagation });
      state.runs = restored.runs;
      for (const run of state.runs) {
        run.propagation = migratePropagation(run.propagation || {});
        if (!(run.regions || []).length) run.regions = [...state.regions.values()].map(r => clone(r));
        if (!run.summary && run.history.length) {
          try { run.summary = analyzeRun(run, new Map(run.regions.map(r => [r.id, r])), state.analysisOptions); } catch (error) { console.warn('Não foi possível recalcular o resumo da execução importada', run.id, error); }
        }
      }

      const requestedRunId = String(data.selectedRunId || '');
      state.selectedRunId = state.runs.some(r => r.id === requestedRunId) ? requestedRunId : (state.runs[0]?.id || null);
      state.viewingRunId = null;
      const selected = state.currentRun();
      const requestedTime = Math.max(0, Math.floor(Number(data.currentTimeStep) || 0));
      state.currentTimeStep = selected ? Math.min(requestedTime, Math.max(0, selected.history.length - 1)) : 0;
      state.selectedRegionId = null;
      state.interactionMode = INTERACTION_MODES.SELECT;
      state.pendingSourceId = null;
      state.pendingPathId = null;
