...
class App {
  ...
  syncSpaceControls() { const mode = this.pendingSpaceMode || state.space.mode; $$('.space-mode-card').forEach(b => b.classList.toggle('active', b.dataset.spaceMode === mode)); $('#grid-space-fields').hidden = mode !== SPATIAL_MODES.GRID; $('#synthetic-space-fields').hidden = mode !== SPATIAL_MODES.SYNTHETIC_MAP; $('#geojson-space-fields').hidden = mode !== SPATIAL_MODES.GEOJSON; const syn = state.space.synthetic || {}; $('#synthetic-count').min = SYNTHETIC_MAP_DEFAULTS.MIN_REGIONS; $('#synthetic-count').max = SYNTHETIC_MAP_DEFAULTS.MAX_REGIONS; $('#synthetic-count').value = syn.regionCount ?? SYNTHETIC_MAP_DEFAULTS.REGION_COUNT; $('#synthetic-seed').value = syn.spatialSeed ?? SYNTHETIC_MAP_DEFAULTS.SPATIAL_SEED; $('#synthetic-irregularity').value = syn.irregularity ?? SYNTHETIC_MAP_DEFAULTS.IRREGULARITY; $('#synthetic-pop').value = syn.defaultPopulation ?? SPATIAL_DEFAULTS.DEFAULT_POPULATION; $('#geojson-default-pop').value = SPATIAL_DEFAULTS.DEFAULT_POPULATION; let label = ''; if (state.space.mode === SPATIAL_MODES.GRID) label = `Grid · ${state.grid.rows} × ${state.grid.columns} · ${state.regions.size} regiões`; else if (state.space.mode === SPATIAL_MODES.SYNTHETIC_MAP) label = `Mapa sintético · ${state.regions.size} regiões · seed espacial ${state.space.synthetic?.spatialSeed}`; else label = `Mapa importado · ${esc(state.space.geojson?.sourceName || 'GeoJSON')} · ${state.regions.size} regiões${state.space.geojson?.stats ? ` · ${state.space.geojson.stats.isolated} isoladas` : ''}`; $('#active-space-summary').innerHTML = `<strong>Espaço ativo</strong><span>${label}</span>`; $('#grid-dim-label').textContent = state.space.mode === SPATIAL_MODES.GRID ? `Grid · ${state.grid.rows} × ${state.grid.columns}` : state.space.mode === SPATIAL_MODES.SYNTHETIC_MAP ? `Mapa sintético · ${state.regions.size}` : `Mapa · ${state.regions.size} regiões`; this.toggleAdjacencyOptions(); }
  ...
  applyGeoJSON() { if (!this.pendingGeoJSON) { this.toast('warning', 'Nenhum mapa selecionado', 'Escolha um arquivo GeoJSON primeiro.'); return; } try { const mapping = { idProperty: $('#geojson-id-prop').value, nameProperty: $('#geojson-name-prop').value, populationProperty: $('#geojson-pop-prop').value }, regions = normalizeGeoJSON(this.pendingGeoJSON, mapping, { defaultPopulation: Math.max(1, num('#geojson-default-pop', GEOJSON_DEFAULTS.DEFAULT_POPULATION)) }), topology = buildGeoJSONTopology(regions, { method: $('#map-adjacency-method').value, tolerance: $('#map-border-tolerance').value, nearestK: num('#map-nearest-k', 4) }); state.applyGeoJSONRegions(topology.regions, { sourceName: this.pendingGeoJSONName, mapping, topology: { method: topology.method, tolerance: topology.tolerance, nearestK: num('#map-nearest-k', 4) }, stats: topology.stats }); this.syncControls(); this.renderer.fit(); this.toast(topology.stats.isolated ? 'warning' : 'success', 'Mapa importado', `${topology.stats.regions} regiões · grau médio ${topology.stats.meanDegree.toFixed(1)} · ${topology.stats.isolated} regiões isoladas.`); } catch (error) { console.error(error); this.toast('danger', 'Falha ao construir o mapa', error.message); } }
  ...
  renderRunConfiguration(run) {
    ...
    box.innerHTML = [
      ...
      item('Caminhos', propagation.pathRegions?.length || 0), item('Direção', direction)
    ].join('');
  }
